...
pub async fn get_id_of_a_new_debate(
    app: &TestApp,
    tournament_id: &str,
...
pub async fn create_debate(
    ...
    app.client
        .post(app.url(&format!("/tournaments/{}/debates", tournament_id)))
        .json(&request_body)
...
pub async fn get_debate(
    ...
    app.client
        .get(app.url(&format!("/tournaments/{}/debates/{}", tournament_id, id)))
...
pub async fn patch_debate(
    ...
    app.client
        .patch(app.url(&format!("/users/{}/debates/{}", judge_id, id)))
        .json(&request_body)
        .header("accept", "text/plain")
        .header("Content-Type", "application/json")
        .bearer_auth(token)
...
pub async fn delete_debate(
    ...
    app.client
        .delete(app.url(&format!("/users/{}/debates/{}", judge_id, id)))
        .header("accept", "text/plain")
        .bearer_auth(token)
        .send()
        .await
