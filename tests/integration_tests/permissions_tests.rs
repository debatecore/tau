...
async fn user_with_organizer_role_can_check_permission_they_have() -> Result<(), OmniError>
{
    ...
    let user_id =
        get_id_of_a_new_user(&app, &Uuid::now_v7().to_string(), "password").await;
...
async fn infrastructure_admin_has_all_permissions() -> Result<(), OmniError> {
    ...
    let response2 =
        check_permission(&app, &admin_id, &tournament_id, "WriteTeams", &admin_token)
            .await;
...
async fn user_not_assigned_to_tournament_returns_401() -> Result<(), OmniError> {
    ...
    let tournament_alpha_id =
        get_id_of_a_new_tournament(&app, "tournament alpha").await?;
