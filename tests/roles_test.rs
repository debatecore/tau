...
#[tokio::test]
#[serial]
async fn admin_should_be_able_to_assign_roles() {
    // GIVEN
...
#[tokio::test]
#[serial]
async fn organizers_should_be_able_to_assign_roles() {
    // GIVEN
    setup::read_environmental_variables();
    setup::check_secret_env_var();
    let state = setup::create_app_state().await;
    ...
    let organizer_handle = "some organizer";
    ...
    let tournament_id = create_tournament("some tournament", "st", &token)
        ...
        .unwrap()
        .to_owned();
    create_roles(&organizer_id, &tournament_id, vec![Role::Organizer], &token).await;
    let organizer_token = get_session_token_for(organizer_handle, organizer_password)
        .await
        .unwrap();
    let response = create_roles(
        &user_id,
...
#[tokio::test]
#[serial]
async fn granting_duplicate_roles_should_cause_conflicts() {
    // GIVEN
    setup::read_environmental_variables();
    setup::check_secret_env_var();
    let state = setup::create_app_state().await;
...
#[tokio::test]
#[serial]
async fn roles_should_be_visible_to_other_tournament_users() {
    // GIVEN
    setup::read_environmental_variables();
    setup::check_secret_env_var();
    let state = setup::create_app_state().await;
...
#[tokio::test]
#[serial]
async fn roles_should_be_modifiable() {
    // GIVEN
    setup::read_environmental_variables();
    setup::check_secret_env_var();
    let state = setup::create_app_state().await;
...
#[tokio::test]
#[serial]
async fn roles_should_be_deletable() {
    // GIVEN
    setup::read_environmental_variables();
    setup::check_secret_env_var();
    let state = setup::create_app_state().await;
